...
\dnote{add back the terminology section (need witness / instance at least), (internal vs external variables)}
...
Going beyond R1CS, support other constraint system representations that are native to some ZK backends. These include Boolean circuits, arithmetic circuits, and algebraic constraint systems.

\subparhead{Uniform constraint systems}
Some backends can take advantage of uniformity in the constraint system (e.g., when some elements are repeated many times). Support the expression such uniformity.
...
\subparhead{Typing} Enforcing properties of variables using a type system (e.g.: boolean type that enforces booleanity check).
